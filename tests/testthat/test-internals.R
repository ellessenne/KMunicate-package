...
library(simsurv)
testfit <- function() {
  # Simulate a trial with 1000 individuals, 1:1 random allocation and a log-treatment effect of -0.5
...
testthat::test_that("Results from .fortify and ggfortify::fortify are consistent", {
  library(broom)
  for (ii in seq(100)) {
    f <- testfit()
    df <- .fortify(f)
